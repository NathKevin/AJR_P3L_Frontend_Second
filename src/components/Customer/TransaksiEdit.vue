<template>
    <v-main class="transaksi">
        ...
        <v-container>
            ...
            <v-card v-if="done" elevation="10" class="overlap">
                <v-toolbar class="cyan lighten-2" min-height="80">
                    <h2 class=" ml-7 mt-4 text-left">Ubah Transaksi Peminjaman</h2>
                </v-toolbar>
            <template>
                <v-stepper
                    v-model="e6"
...
export default {
    ...
    methods: {
        ...
        async confirmHandler(){
            ...
            await this.updatePembayaran();
        },
        async updatePembayaran(){
            var url = this.$api + '/update/pembayaran/' + this.transaksi.idPembayaran;
            await this.$http.put(url, this.allPembayaranForm, {
                  headers: {
                      'Authorization' : 'Bearer ' + sessionStorage.getItem('token')
                  }
            }).then(response => {
        ...
        async transaksiHandler(){
            ...
            await this.updateTransaksi();
        },
        async updateTransaksi(){
            var url = this.$api + '/update/transaksi/' + this.transaksi.idTransaksi;
            await this.$http.put(url, this.allTransaksiForm, {
                  headers: {
                      'Authorization' : 'Bearer ' + sessionStorage.getItem('token')
                  }
            }).then(response => {
        ...
        async getDriverAvailable(){
         ...
         async showTransaksi(){
            var url = this.$api + '/showUbah/transaksi/' + sessionStorage.getItem('id');
            await this.$http.get(url, {
                  headers: {
                      'Authorization' : 'Bearer ' + sessionStorage.getItem('token')
                  }
                ...
                this.getAllPromo();
                this.getMobilAvailable();
                this.getDriverAvailable();
            })
        },
        async getPromo(){
            if(this.transaksi.idPromo != null){
                console.log(this.promosAvailable[1].idPromo)
                for(let i=0; i<this.promosAvailable.length; i++){
                    console.log(this.promosAvailable[i].idPromo)
                    if(this.promosAvailable[i].idPromo == this.transaksi.idPromo){
                        console.log('test')
                        this.formPembayaran.promo = this.promosAvailable[i].idPromo
                        console.log(this.formPembayaran.promo)
                    }
                }
                if(this.formPembayaran.promo == null ){
                    for(let i=0; i<this.promosAvailable.length; i++){
                        if(this.promosAvailable[i].jenisPromo == "None"){
                            this.formPembayaran.promo = this.promosAvailable[i].idPromo
                        }
                    }
                }
        ...
        async detailHandler(){
            this.formTransaksi.tanggalWaktuSewa = this.transaksi.tanggalWaktuSewa;
            this.formTransaksi.tanggalWaktuSelesai = this.transaksi.tanggalWaktuSelesai;
            await this.getPromo();
            this.formPembayaran.metodePembayaran = this.transaksi.metodePembayaran;
            this.selectedMobil = this.transaksi.idMobil;
            this.formSelectedMobil.namaMobil= this.transaksi.namaMobil;
            this.formSelectedMobil.gambarMobil = this.transaksi.gambarMobil;
            this.formSelectedMobil.tipeMobil = this.transaksi.tipeMobil;
            this.formSelectedMobil.jenisTransmisi = this.transaksi.jenisTransmisi;
            this.formSelectedMobil.jenisBahanBakar = this.transaksi.jenisBahanBakar;
            this.formSelectedMobil.kapasitasPenumpang = this.transaksi.kapasitasPenumpang;
            this.formSelectedMobil.fasilitas = this.transaksi.fasilitas;
            this.formSelectedMobil.hargaSewaMobil = this.transaksi.hargaSewaMobil;
            this.selectedDriver = this.transaksi.idDriver;
            if(this.selectedDriver != null){
                this.isDriver = 'Pakai'
                this.formSelectedDriver.namaDriver = this.transaksi.namaDriver;
                this.formSelectedDriver.jenisKelaminDriver = this.transaksi.jenisKelaminDriver;
                this.formSelectedDriver.noTelpDriver= this.transaksi.noTelpDriver;
                this.formSelectedDriver.hargaSewaDriver = this.transaksi.hargaSewaDriver;
                this.formSelectedDriver.rerataRating = this.transaksi.rerataRating;
                this.formSelectedDriver.fotoDriver= this.transaksi.fotoDriver;
            }else{
                this.idDriver = 'Tidak Pakai'
            }
            this.load = false;
            this.done = true;
        },
